import { getAllInsightBlogs, getinsightBlogBySlug } from '@/lib/insightBlogData';
...
interface InsightBlogProps {
  ...
  export async function generateStaticParams() {
    const blog = await getAllInsightBlogs(); // Use the function to fetch all blog posts
  ...
  export async function generateMetadata({ params }: InsightBlogProps): Promise<Metadata> {
    const blog = await getinsightBlogBySlug(params.slug);
    if (!blog) {
      return {
