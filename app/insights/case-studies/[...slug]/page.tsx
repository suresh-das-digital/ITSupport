import { getAllInsightCaseStudy, getinsightCaseStudyBySlug } from '@/lib/insightBlogData';
...
interface InsightBlogProps {
  ...
  export async function generateStaticParams() {
    const blog = await getAllInsightCaseStudy(); // Use the function to fetch all blog posts
  ...
  export async function generateMetadata({ params }: InsightBlogProps): Promise<Metadata> {
    const blog = await getinsightCaseStudyBySlug(params.slug);
    if (!blog) {
      return {
