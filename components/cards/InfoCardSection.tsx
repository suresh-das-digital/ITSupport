      ...
      <div className="max-w-6xl 2xl:max-w-[1488px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cards.map((card, index) => (
            ...
            >
              <div className="h-full w-full cursor-pointer p-6 bg-[#ffffff] group-hover:bg-white/50 duration-75">
                <h3 className="text-lg font-semibold text-gray-900 mb-2 group-hover:text-[#61CE70] duration-0">
                  {card.title}
                </h3>
                <p className="text-gray-700 text-sm">{card.desc}</p>
              </div>
              <div className="absolute blur-sm top-0 left-[35%] md:left-[50%] lg:left-[43%] w-full h-full text-[24em] leading-0 translate-y-1/2 -translate-x-1/2 opacity-0 duration-75 group-hover:opacity-50 -z-1">
                <span>{card?.flag}</span>
              </div>
            </div>
          ))}
